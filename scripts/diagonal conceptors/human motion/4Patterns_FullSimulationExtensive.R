...
if(iterative) learning_rate <- 0.5
...
for(p in 1:n_pattern){
  ...
  for(n in 1:n_run[p]){
    ...
    if(n > n_washout[p]){
      # compute output
      p_collector[,n - n_washout[p]] <- W_out %*% r
    }
...
# phase align patterns
phase_aligned_patterns <- phaseAlignPatterns(observed=sg_outputs, target=patterns)
...
for(i in 1:nrow_plots^2){
  if(show_plots){
    target_i <- phase_aligned_patterns[[k_pattern]]$target_pattern[,i]
    observed_i <- phase_aligned_patterns[[k_pattern]]$observed_pattern[,i]
    plot(target_i, type = 'l', ylim = c(-1,1), ylab = '')
    lines(observed_i, col=2)
    nrmse_i <- nrmse(target_i, observed_i)
    legend('bottomleft', legend=c(paste0('error=',round(nrmse_i,3)))) 
  }
}
# compute errors
nrmses <- lapply(phase_aligned_patterns, function(x) x$nrmse)
for(p in 1:n_pattern){
