...
if(H){
  ...
  # apertures
  apertures <- c(25,50,17,49,89,49,9,65,49,9,25,25,17,9,7)
}else{
  # scaling
  W_in_scaling <- 0.2
  b_scaling <- 0.8
  ...
  # apertures
  apertures <- c(25,50,17,49,89,49,9,65,49,9,25,25,17,9,33)
}
...
# apertures <- c(25,50,17,49,89,49,9,65,49,9,25,25,17,9,7)
# Cs <- list()
# Cs[[1]] <- computeConceptor(R[[1]],25)
# Cs[[2]] <- computeConceptor(R[[2]],50)
# Cs[[3]] <- computeConceptor(R[[3]],17)
# Cs[[4]] <- computeConceptor(R[[4]],49)
# Cs[[5]] <- computeConceptor(R[[5]],89)
# Cs[[6]] <- computeConceptor(R[[6]],49)
# Cs[[7]] <- computeConceptor(R[[7]],9)
# Cs[[8]] <- computeConceptor(R[[8]],65)
# Cs[[9]] <- computeConceptor(R[[9]],49)
# Cs[[10]] <- computeConceptor(R[[10]],9)
# Cs[[11]] <- computeConceptor(R[[11]],25)
# Cs[[12]] <- computeConceptor(R[[12]],25)
# Cs[[13]] <- computeConceptor(R[[13]],17)
# Cs[[14]] <- computeConceptor(R[[14]],9)
# Cs[[15]] <- computeConceptor(R[[15]],7)
# compute conceptors
...
# pa <- phaseAlignPatterns(sg_outputs, target_outputs)
...
#   plot(pa[[k_pattern]]$target_pattern[,i], type = 'l', ylim = c(-1,1), ylab = '')
#   lines(pa[[k_pattern]]$observed_pattern[,i], col=2, lwd=2)
#   
#   #nrmse_1 <- nrmse(target_outputs[[k_pattern]][,i], sg_outputs[[k_pattern]][,i])
#   #legend('bottomleft', legend=c(paste0('error=',round(nrmse_1,3))))
# }
# # errors
# nrmses <- lapply(pa,function(x)x$nrmse)
# for(p in 1:n_pattern){
...
for(i in 1:nrow_plots^2){
  ...
  lines(sg_outputs[[k_pattern]][,i], col=2, lwd=2)
...
if(save_results){
  ...
  if(H){
    file_name_reservoir <- 'output/reservoirs/reservoir_c_hm_H.RData'
  }else{
    file_name_reservoir <- 'output/reservoirs/reservoir_c_hm_J.RData'
  }
